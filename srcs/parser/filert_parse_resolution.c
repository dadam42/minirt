...
t_filert_parser_com	filert_parse_resolution(char **str, t_filert_parsed_obj *obj)
{
	...
	if (filert_parse_int(str, &obj->resolution.x) == filert_parser_error)
		return (filert_error);
	if (obj->resolution.x <= 0)
		return (filert_error);
	if (filert_parse_str(str, FILERT_SEP_STR) == filert_parser_error)
		return (filert_error);
	if (filert_parse_int(str, &obj->resolution.y) == filert_parser_error)
		return (filert_error);
