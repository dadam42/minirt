...
short	int_sphere_ray(t_geom_sphere *sphere, t_ray *ray)
{
	t_vec3	tmp;
	float	b;
	float	c;
...
void	t_object_dummy_get_color(t_object *obj, t_local_coord coord, t_color color)
{
	(void)coord;
	ft_memcpy(color, obj->color, sizeof(t_color));
}

void	t_sphere_get_more_intersection_info(t_object* obj, t_intersection_info *info)
{
...
int main()
{
	t_base	canonical = {{1, 0, 0},{0, 1, 0},{0,0,1}};
	t_camera camera = {.position = {0,-4,0}, .base[view] = {0,1,0}, .base[right] = {1, 0,0}, .fov = 60};	
	t_object sphere0 = {.geom={.sphere = {.center = {0,0,0}, .radius = 2}}
							, .color = {1, 0, 0}
							, .get_intersection_info = t_sphere_get_intersection_info};
	t_light light0 = {.position = {6,0,0}, .pcolor = {.intensity = 4000, .color = {0, 1, 0}}};
	t_minirt minirt = {.scene = {.nobjects = 1, .nlights = 1},
						.camera = &camera,
						.resolution = {.height = 1000 ,.width = 1000},
						};
	t_ambiant_light ambiant_light = {.color={1,1,1}, .intensity=2};
	t_color background_color = {0,0,0};
	t_com ret;
	
	t_object **sphere_obj = malloc(minirt.scene.nobjects * sizeof(t_object*));
	t_vec3_vprod(camera.base[right], camera.base[view], camera.base[up]);
	ft_memcpy(sphere0.base, canonical, sizeof(t_base));
	sphere_obj[0] = &sphere0;
	minirt.scene.objects = sphere_obj;
	ft_memcpy(&minirt.scene.ambiant_light, &ambiant_light, sizeof(t_ambiant_light));
	ft_memcpy(&minirt.scene.background_color, &background_color, sizeof(t_color));
	t_light **lights = malloc(minirt.scene.nlights * sizeof(t_light*));
	lights[0] = &light0;
	minirt.scene.lights = lights;
	char	filename[] = "display.bmp";

	ret = t_save_bmpfile(&minirt, filename);
	if (ret == minirt_ok)
...
/*void	t_ray_init(t_ray *ray, t_position position, t_direction direction)
{
	ft_memcpy(ray->start, position, sizeof(t_position));
	ft_memcpy(ray->direction, direction, sizeof(t_direction));
}
...
/*void	t_get_light_influence(t_minirt *minirt, t_intersection_info *info, t_pcolor *light_influence)
{
	int							cur_light;
	int							cur_object;
	...
	ft_memcpy(ray.start, info->coord.position, sizeof(t_position));

	ft_memcpy(light_influence, &minirt->ambiant_light, sizeof(t_pcolor));
	...
	while(cur_light < minirt->scene.nlights)
	{
		...
		t_intersection_info_init(&cur_info, &ray);
		cur_object = 0;
...
/*void	t_get_color(t_minirt *minirt, t_ray *ray, t_color color)
{
	t_intersection_info	info;
	t_pcolor				light_influence;
	int		cur_obj;
	
	cur_obj = 0;
	t_intersection_info_init(&info, ray);
	...
	if (info.object)
	{
		...
		t_get_light_influence(minirt, &info, &light_influence);
