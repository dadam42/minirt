...
void	scene_get_light_influence(t_scene* scene, t_intersection *inter)
{
	...
	{
		distcoef = ray_init(&shadow_ray, inter->position
						, light.deref(&light)->position);
		intersection_init(&shadow_inter, scene, &shadow_ray, distcoef);
